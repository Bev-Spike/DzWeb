...
const char *error_403_form = "You do not hava permission to get file from this server.\n";
const char *error_404_title = "Not Found";
const char *error_404_form = "The requested file was not found on this server.\n";
const char *error_500_title = "Internal Error";
const char *error_500_form = "There was an unusual problem serveing the requested file.\n";
...
//分析请求行（第一行）例如 GET http://127.0.0.1 HTTP/1.1
//temp为请求行字符串的首地址
http_conn::HTTP_CODE http_conn::parse_request_line(char* text){
    ...
    *_url++ = '\0';
    ...
    _url+= strspn(_url, " \t");//该函数返回第一个不是空格和\t的字符的下标,即跳过空格和\t字符
...
http_conn::HTTP_CODE http_conn::process_read(){
    ...
    while(((_check_state == CHECK_STATE_CONTENT) && (line_status == LINE_OK)) || 
        ...
        case CHECK_STATE_CONTENT:{
            ...
            //当请求体不完整的时候，仍然需要继续读取数据
            //只是简单地将读取行状态修改为OPEN时，该状态机会继续向后面解析行，可能会出现不可预知的情况，我这里直接让他返回了
